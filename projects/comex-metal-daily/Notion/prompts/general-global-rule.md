...
*因为预先建好结构，才能避免 Agent 乱放文件、随手新建文件夹，导致项目越长越乱。*
...
- **派生场景同样适用**：watchdog、cron 脚本、subagent、临时脚本、"为了简便先放着以后改"——**全部不行**。
- 写新脚本前的检查 0 步：`grep -E "ntn_|sk-|AQ\\.|7[0-9]{9}:[A-Za-z0-9_-]+|api_key.*=|token.*=" <脚本>` 看有没有疑似密钥字面值。命中 → 立刻改 env var。
- 凭证已曝光时（包括在用户聊天、agent 之间通信、log 文件）→ **立刻在 cowork 报警 + 提醒用户 rotate**。不要假设"反正只有可信方看到"。
...
- **2026-05-31 Hermes 第 1 次硬编码 NOTION_TOKEN**：`Daily_GoldSilvPT-inv_Notion/sync_shfe.py` 把 `ntn_193057252443LthT...` 写死在脚本里 + launchd plist 又写一次（双写）。cowork Claude 提醒过 1 次，要求 rotate 但 token 当时未 rotate。
- **2026-06-06 Hermes 第 2 次硬编码 NOTION_TOKEN**：`Comex Metal Daily Issue Report/scripts/comex_watchdog.py` 又把同一 token 写死。**同一条 §7 第 1 项规则连续违反 2 次**。已主动改为 `os.environ["NOTION_TOKEN"]`，从 plist EnvironmentVariables 读取。
- **下次再犯（第 3 次）→ 用户撤销 Hermes 自治新建脚本的权限**：每次新建脚本必须先在 cowork 提交内容审批，确认无硬编码 secrets 才能落盘。

---

## §9 文件系统纪律（写在哪 / 不写在哪）

**写文件前必须 realpath 验证**：任何 `Write` / `mkdir` / 脚本输出之前，确认目标路径**不在用户私人文件夹**。这是 §2.6"落笔前先读"与 §2.10"显式失败"在文件系统层的延伸。

### 绝对禁区（无论何种"看上去合理"的理由）

- `~/Documents/`（**包括任何子文件夹**——新建 `Documents/Claude/`、`Documents/<agent-name>/`、`Documents/<project-output>/` 这种"自给自足子目录"也算违规）
- `~/Desktop/` `~/Downloads/` `~/Pictures/` `~/Movies/` `~/Music/`
- 任何系统目录（`/etc /opt /usr /System`）

### Claude Code 的默认行为不豁免规则

- Claude Code 默认会把 scheduled skill 写到 `~/Documents/Claude/Scheduled/`——**必须立刻 `mv` 到对应项目的 `scheduled-tasks/` 子目录**（如 `~/hermesagent/Youtube video/scheduled-tasks/`），然后告诉用户
- Claude Code 默认会把 session memory 写到 `~/Library/Application Support/Claude/`——这是例外（Anthropic 内部存储不计违规）

### 正确的写入位置

| 类型 | 位置 |
|---|---|
| 项目产物（输出 / 中间数据 / 日志） | `~/hermesagent/<project-name>/{outputs,scratch,data,logs}/` |
| Profile 工作区 | `~/hermesagent/<profile-name>/`（finance / general / 未来 hero） |
| 全局规则/协议文档 | `~/hermesagent/Hermes General Rule & Protocol/` |
| Wiki 知识 / 教训 | `~/Antigravity Projects/Generalrule/wiki/` |
| Antigravity 项目族 | 同族项目集中在父目录（COMEX → `~/Antigravity Projects/COMEX-Metal-Daily/`），**禁止散落根目录** |
| 临时一次性（脚本中间文件 / 重启就丢） | `/tmp/` |

**不知道往哪放 → 停下来问用户**（§2.10 Fail Loud 的延伸，不允许"先放一个看上去合理的地方等以后整理"）。

**完整决策树见** `wiki/agent-rules/hermes-profile-filesystem-discipline.md`。

### 违规历史教训（不可重复）

- **2026-05-31 Hermes 文件系统纪律违规**：Hermes 把 895MB 杂志 video 写到 `~/Documents/hermes_output/`、webworms.md 写到 `~/Documents/Claude/Projects/Hermes开发汇总/`、2 个 scheduled SKILL 写到 `~/Documents/Claude/Scheduled/`。**规则本就存在**(CLAUDE.md §"根目录写入禁令" + 本文件 §7)，Hermes 仍然违反。用户人工 mv 全部到 `~/hermesagent/Youtube video/{magazine_outputs,skills,scheduled-tasks}/` 才纠正。**下次再犯 → 用户撤销部分自治权限，要求每次写文件先在 cowork 审批**。
- **2026-05-31 Antigravity 散落项目违规**：5 个 COMEX 相关 git repo 散落在 `~/Antigravity Projects/` 根目录，而非集中在 `COMEX-Metal-Daily/` 父目录。已要求 Antigravity 重组到 `~/Antigravity Projects/COMEX-Metal-Daily/{Daily_*,Weekly_*,comex-hermes-ingestor}/`。**未来同族项目的第 2+ 个 repo 必须放进既有父目录，第 1 个 repo 时跟用户确认是否需要建父目录**。
...
| 文件系统纪律（写在哪/不写在哪/profile 工作区结构） | `wiki/agent-rules/hermes-profile-filesystem-discipline.md` |
