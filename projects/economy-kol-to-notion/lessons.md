...
## 2. 搜索源选型
- usearch/Glean（aifx）是**内部知识检索**，搜不到 Kitco/YouTube/X 等公网 KOL 观点 → 不用于 KOL 搜索。
- aifx mcp list 里有 `playwright`（浏览器自动化）但 `aifx mcp call playwright` 报 404（local server，调法待解）。
...
- 安装：`npm install -g agent-browser --registry=https://registry.npmjs.org/`（VM 默认 npm registry 是 `unpm.uberinternal.com` 需认证会 401，必须显式指定公网 registry）→ 然后 `agent-browser install --with-deps`（下载 Chrome for Testing 150 + CJK/emoji 字体）。
...
3. **正解 = Google News 官方 RSS**：`https://news.google.com/rss/search?q=<url编码>&hl=en-US&gl=US&ceid=US:en`，纯 HTTP(xml.etree 解析 item 的 title/link/description/pubDate)。完全独立于 SearXNG/Cerberus/付费 key、不反爬、**自带精确 pubDate**（弥补 SearXNG 无日期短板）。这才是真"直接交给 Google"。
...
5. **Uber 内部无公网搜索 MCP**（穷尽确认）：usearch/usearch-gen-ai=内部知识库、google-mcp=Google Workspace(Gmail/Docs 非搜索)、opensearch/sourcegraph/wayfare 全是内部/业务。**Claude Code WebSearch 在 Uber 网关下返回 `no websearch`**（Anthropic server-side 工具不透传）。所以没有现成的内部公网搜索可当兜底。
