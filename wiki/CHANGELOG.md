...
### 2026-07-02 —— Hindsight local_embedded 记忆后端旧 glibc 三坑（Hermes, Opus 4.8 [UB]）

- **[新增] `engineering/hindsight-local-embedded-pitfalls.md`** —— 在 Debian 12 / glibc 2.36 上启用 Hermes hindsight 记忆后端（local_embedded + hybrid），从 `hermes memory status` 报 `not available` 排到 `available ✓`，中途撞三个互相独立、错误信息互相掩盖的坑，全实测修复后沉淀。**核心知识**：①**坑1 pgvector glibc 不兼容（最硬核）**——pg0 内嵌 postgres 打包的预编译 `vector.so` 需 `GLIBC_2.38`、主机仅 2.36，加载失败→`CREATE EXTENSION vector` 失败→`Database migration failed`。修法=**用 pg0 自带 `pg_config` 从 pgvector 源码本地重编译**（`make PG_CONFIG=$PGROOT/bin/pg_config`），重编后只需 GLIBC_2.14；SQL 迁移文件本就在，只有 `.so` 二进制不对。②**坑2 缺 uvx**——daemon 找不到 venv 本地入口时 fallback `uvx hindsight-api@ver`，机器没 uv 直接死。修=`pip install hindsight-api` 装进 Hermes 同 venv（让 sysconfig 解析到入口）+ 装 uv 兜底；启动前须 `export PATH=$HOME/.local/bin:$PATH`。③**坑3 顶层 `hindsight` 包不存在**——插件 `_check_local_runtime` 做 `import hindsight` + `from hindsight import HindsightEmbedded`，但 0.8.x 布局只有 `hindsight_client/_embed/_api`，import 抛错→status 永假→真实 session 静默 fallback 到内置 MEMORY.md（灌的记忆白灌）。死路：`pip install hindsight`（是无关 Py2 死包 `use_2to3 invalid`）、降级 client 0.6.1（也无此类）。修=**在 `<venv>/site-packages/hindsight/__init__.py` 写 compat shim**，`HindsightEmbedded` 内部 `ensure_running`+`get_url` 管 daemon、包装 `hindsight_client.Hindsight`、暴露 async `a*` 方法+`.url`（插件 `await operation(client)` 且读 `client.url` 是载荷契约）。**通用教训**：一个 `not available` 可叠三个不相干根因，别看到第一个错就以为修完；**CLI 横幅会残留上一轮旧错，须以带时间戳的日志文件为准**；`daemon start` 前台挂起须后台启+独立验证；预编译 native 扩展遇旧发行版 glibc，本地重编译比换栈更省事。交叉链接 [[container-reboot-service-persistence]]、[[rag-chatbot-first-build-pitfalls]]、[[rag-incremental-index-refresh]]。同步登记 `engineering/README.md`（engineering 域已存在，index 第一层无需改）。不含 Uber 专有内容=纯通用技术。
