...
| 持久记忆 | Auto Memory：`~/.claude/projects/<slug>/memory/` md+frontmatter+`MEMORY.md` 索引 | **无原生**（靠 Lesson+llm-wiki 手动写 wiki / 各项目 `docs/context-log.md`） | `memories/MEMORY.md`+`USER.md`（§分隔无 frontmatter）+ 向量层（hindsight/holographic SQLite） | SQLite：`memories_1.sqlite`/`state_5.sqlite`/`sessions/`（memory_mode 默认 enabled） | **无**可自管理持久层 |
...
- **记忆**：**无原生持久层**。当前靠 Lesson 系统 + llm-wiki skill 手动写 Wiki（自动上传不成功）。Plan 上下文靠各项目 `docs/context-log.md`（及 `tasks/lessons.md`、`tasks/todo.md` 等，Hermes 留下的）。= 无自动跨 session 检索。
