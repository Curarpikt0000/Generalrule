...
| `hermes-profiles/handbook/`（蒸馏议会方法论） | Hermes 议会模式人格蒸馏完整方法论：5 条铁律 + 4 层议会架构 + 阵容设计原则 + 大师 SKILL 蒸馏 SOP + 蓝图↔profile SSOT 同步机制 + 部署流程 + 23 个踩坑教训。两套可复制的实例（Finance Hero 11 位投资大师 / General Hero 10 位伟人 + 四件套脚手架） | 自有 Cowork 蒸馏实践沉淀 | 阅读 `hermes-profiles/handbook/DISTILLATION-HANDBOOK.md` 理解全貌，急用看 `QUICK-START.md` 按 checklist 执行。需要新 profile 时，复制 `templates/` 中的 SOUL.template / master-SKILL.template / depth-modes.template 等填空 |
