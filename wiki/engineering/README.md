...
* [[uber-query-writing-and-schema-verification]] - Uber query 编写 & 表 schema 查证合集（**queryrunner 正确路由参数是 `datasource:presto` 非 cluster_name/secure**；`SHOW COLUMNS` failure≠表不存在，先 usearch 查改名如 plan_fx_rates_japan_latest→plan_fx_rates_2026_v2；Databook local MCP 绕 Presto 拥堵；Notion 编目字段/评分格式）。**Uber agent 写 query 必须先调 skill `jpgr-query-writer`**（ub-branch / Uber 内部）。
...
* [[hindsight-local-embedded-pitfalls]] - Hindsight local_embedded 记忆后端在旧 glibc Linux 的三个叠加坑（**pgvector `vector.so` 需 GLIBC_2.38 而主机仅 2.36→用 pg0 的 pg_config 从源码重编译**降到只需 2.14；`uvx` 缺失→装 hindsight-api 到 venv + 装 uv 兜底；**顶层 `hindsight` 包不存在导致 `_check_local_runtime` 永假、status 静默 fallback→写 compat shim 补 `HindsightEmbedded`**；CLI 横幅会残留旧错须以带时间戳日志为准；daemon 前台 start 挂起须后台启+独立验证）
