...
5. **搜索源优先级（精度递减降级链，backfill_one.py 已内置）**：**Exa（主，带日期窗最准）→ GenAI Gateway grounded search（Google引擎，免费不断粮，带源URL，质量高于ddgs）→ ddgs（完全免费兜底，`pip install ddgs --break-system-packages`）**。Tavily 保留在链中（充值后自动复活）但当前超额。触发：付费源(Exa+Tavily)命中=0时自动启用 GenAI，GenAI 也挂才到 ddgs。输出 json 有**四桶** `exa`/`tavily`/`genai`/`ddgs`，**下游分析/cron prompt 必须四桶都读**否则断粮当天观点全丢。genai 桶第一条是 Gemini 综述正文(信息密度最高)、后续是源URL；genai/ddgs 无 date 字段时效靠正文判断。
   - ⭐**Uber 内部公网联网方案（2026-06 实测，纠正旧结论"内部无公网搜索"）**：`aifx mcp list` 没有独立 web/news/serp MCP，但**官方公网出口收口在 GenAI Gateway**。①【已集成为首选兜底】**GenAI Gateway grounded search**：`POST http://localhost:5436/v1/models/gemini-2.5-flash:generateContent`，头 `Rpc-Service: genai-api` + `Rpc-Caller: <ldap>@uber.com`，body 加 `"tools":[{"google_search":{}}]`；返回 `candidates[].content.parts[].text`(综述) + `groundingMetadata.groundingChunks[].web.uri`(源URL)。**免费走内部计费、不断粮**。OpenAI 版 `/v1/responses` tools `[{"type":"web_search"}]`；Anthropic `/v1/messages` tools `[{"type":"web_search_20250305"}]`。②LangFx `get_tools(["search.web"])` / `search.serpapi`（Uber 已买 SerpAPI 额度）。③抓全文走 Oxylabs。依赖 localhost:5436 Cerberus 隧道 + ussh 证书(devpod 常驻)。
   - ⚠️**AI-Guard PII 匿名化坑 + 绕过实战**：prompt 里的人名会被替换成 `ANONYMIZED_PERSON_X`。实测："Peter Schiff gold"(带空格的标准全名)→被匿名化返回空；但用 **handle 拼写(去@去空格,如 `PeterSchiff`) + focus 话题(可中文)** 构造查询 → AI-Guard 未匿名化、Gemini 正确识别本人并返回真实当日观点+中文综述。**所以 genai_search 查询拼 `<handle> <focus> market news <start> to <end>`（backfill_one.py 已这么做）**。纯话题查询(无人名)也能拿当日板块动态。
